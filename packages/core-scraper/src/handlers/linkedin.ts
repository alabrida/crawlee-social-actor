...
export async function handle(
    ...
    const isCompanyPage = url.includes('/company/') || url.includes('/school/');
    ...
    try {
        ...
        if (!isBlocked) {
            if (isCompanyPage) {
                ...
                if (await taglineLoc.isVisible()) headline = (await taglineLoc.innerText()).trim();

                const followerLoc = page.locator('.org-top-card-summary-info-list__info-item:has-text("followers"), span:has-text("followers")').first();
                if (await followerLoc.isVisible()) {
                    followerCount = parseCount(await followerLoc.innerText());
                }

                const websiteLoc = page.locator('a.org-top-card-primary-actions__action, a[href*="redir/redirect"], a[href*="company/"] + a').first();
                if (await websiteLoc.isVisible()) {
                    const href = await websiteLoc.getAttribute('href');
                ...
                // About length
                const aboutSec = page.locator('section.org-about-company-module, section:has-text("About")').first();
    ...
    const scrapedItem: ScrapedItem = {
        ...
        data: {
            isPersonalProfile: !isCompanyPage,
            fullName,
            headline,
            location,
            followerCount,
